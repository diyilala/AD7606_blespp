...
static void notify_event_handler(esp_ble_gattc_cb_param_t * p_data)
{
    ...
    if(handle == db[SPP_IDX_SPP_DATA_NTY_VAL].attribute_handle){
...
#else
        /*测试代码20220714，打印接受到的notify的hex格式数据，
          本代码关键修改处，ble_spp只支持字符型和hex的显示，
          接收数据用hex，后续转换成float
          -add by lichen*/
        ...
        if((p_data->notify.value[0] == '#')&&(p_data->notify.value[1] == '#')){
            ...
            //在串口写入server发来的信息，在回复时会显示乱码，这里注释掉
        }
#endif
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_WRITE_DESCR_EVT:
        ...
        case SPP_IDX_SPP_DATA_NTY_VAL:
            ...
            xQueueSend(cmd_reg_queue, &cmd,10/portTICK_PERIOD_MS);//原10/portTICK_PERIOD_MS
            break;
...
void ble_client_appRegister(void)
{
    ...
    esp_err_t local_mtu_ret = esp_ble_gatt_set_local_mtu(400);
    if (local_mtu_ret){
        ESP_LOGE(GATTC_TAG, "set local  MTU failed: %s", esp_err_to_name_r(local_mtu_ret, err_msg, sizeof(err_msg)));
    }

    cmd_reg_queue = xQueueCreate(10, sizeof(uint32_t));
    xTaskCreate(spp_client_reg_task, "spp_client_reg_task", 2048, NULL, tskIDLE_PRIORITY, NULL);
